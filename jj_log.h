...
typedef struct {
  ...
  size_t
      ring_buffer_size; /* Number of slots in ring buffer (0 = default 1024) */
} jj_log_config;
 ...
 * @{
 */
#define jj_log_trace(cat, ...)                                                 \
  jj_log_log_cat(JJ_LOG_TRACE, cat, __FILE__, __LINE__, __VA_ARGS__)
#define jj_log_debug(cat, ...)                                                 \
  jj_log_log_cat(JJ_LOG_DEBUG, cat, __FILE__, __LINE__, __VA_ARGS__)
#define jj_log_info(cat, ...)                                                  \
  jj_log_log_cat(JJ_LOG_INFO, cat, __FILE__, __LINE__, __VA_ARGS__)
#define jj_log_warn(cat, ...)                                                  \
  jj_log_log_cat(JJ_LOG_WARN, cat, __FILE__, __LINE__, __VA_ARGS__)
#define jj_log_error(cat, ...)                                                 \
  jj_log_log_cat(JJ_LOG_ERROR, cat, __FILE__, __LINE__, __VA_ARGS__)
#define jj_log_fatal(cat, ...)                                                 \
  jj_log_log_cat(JJ_LOG_FATAL, cat, __FILE__, __LINE__, __VA_ARGS__)
/* @} */
